...
static void compute3(int * result) {
	for (int i = 0; i < sizeof(list); i++) {
		for (int j = i; j < sizeof(list) - i; j++) {
            for (int k = j; k < sizeof(list) - j; k++) {
                if (list[i] + list[j] + list[k] == 2020) {
                    *result = list[i] * list[j] * list[k];
                    printf("the first number is %d, the second is %d and the third one is %d\n", list[i], list[j], list[k]);
                    return;
			    }
		    }
	    }
